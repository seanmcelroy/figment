...
/// Prioritizes a task.
/// </summary>
public class PrioritizeTaskCommand : CancellableAsyncCommand<PrioritizeTaskCommandSettings>
{
    /// <inheritdoc/>
    public override async Task<int> ExecuteAsync(CommandContext context, PrioritizeTaskCommandSettings settings, CancellationToken cancellationToken)
    {
        ...
        if (ulong.TryParse(settings.TaskNumber, out ulong taskNumber))
        {
                    ...
                    cancellationToken))
            {
                ...
                if (tsr.Success)
                {
                    ...
                    if (saveSuccess)
                    {
                        AmbientErrorContext.Provider.LogDone($"Task #{id.Value.Value} prioritized.");
